...
public class VideoMemoryTest {
    ...
    public void shouldPixelsReflectVideoMemoryLayoutWhenTopByteIsSet() {
        ...
        saveToFile(mem, Path.of("test-files","test001.png").toString());
    }
    ...
    public void shouldPixelsReflectVideoMemoryLayoutWhenMiddleByteIsSet() {
        ...
        mem.setByteAt(0x1800 + 10 + 8*32, Color.getAttribute(Color.CYAN, Color.YELLOW, Color.BRIGHT, Color.NONE));
        mem.setByteAt(0x1800 + 13 + 15*32, Color.getAttribute(Color.WHITE, Color.BLACK, Color.BRIGHT, Color.NONE));
        ...
        verifyOneCell(mem, 6, 0, Color.BLUE, Color.RED, true);
        verifyOneCell(mem, 8, 7, Color.MAGENTA, Color.GREEN, true);
        verifyOneCell(mem, 10, 8, Color.CYAN, Color.YELLOW, true);
        verifyOneCell(mem, 13, 15, Color.WHITE, Color.BLACK, true);
        verifyOneCell(mem, 17, 16, Color.BLUE, Color.GREEN, false);
        verifyOneCell(mem, 31, 23, Color.WHITE, Color.RED, false);

        saveToFile(mem, Path.of("test-files","test002.png").toString());
    }

    private void verifyOneCell(VideoMemory mem, int x, int y, int pen, int paper, boolean bright) {
        int[] pixels = mem.toPixels();
        for(int iy=0; iy<8; iy++) {
            for(int ix=0; ix<8; ix++) {
                assertEquals(Color.getScreenColor(((ix+iy) & 1) == 0 ? paper : pen, bright),
